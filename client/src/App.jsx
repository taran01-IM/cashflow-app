...
import { Icon, useToasts, useConfirm } from './primitives.jsx';
import { useTweaks, TweaksPanel, TweakSection, TweakToggle, TweakColor, TweakRadio } from './TweaksPanel.jsx';
import { BUSINESS_UNITS_FALLBACK } from './data.js';
...
export default function App() {
  ...
  const [businessUnits, setBusinessUnits] = React.useState(BUSINESS_UNITS_FALLBACK);

  const [store, setStore] = React.useState({ banks: [], inflows: [], outflows: [] });

  const [pushToast, toastNode] = useToasts();
  const [confirm, confirmNode] = useConfirm();

  const t = React.useCallback((key) => {
    return I18N[tweaks.language]?.[key] || I18N.en[key] || key;
  }, [tweaks.language]);
  ...
  const loadAll = React.useCallback(async () => {
    const [bus, banks, inflows, outflows] = await Promise.all([
      api.businessUnits(), api.banks(), api.inflows(), api.outflows(),
    ]);
    setBusinessUnits(bus.length ? bus : BUSINESS_UNITS_FALLBACK);
    setStore({ banks, inflows, outflows });
  }, []);

  React.useEffect(() => {
    if (!currentUser) return;
    loadAll().catch((e) => pushToast(e.message || 'Failed to load data'));
  }, [currentUser, loadAll, pushToast]);
  ...
  const visibleBUs = React.useMemo(() => {
    if (!effectiveUser) return businessUnits;
    if (effectiveUser.role === 'Admin') return businessUnits;
    return businessUnits.filter(u => effectiveUser.units.includes(u.id));
  }, [effectiveUser, businessUnits]);
  ...
  const onRequestOtp = (phone) => api.requestOtp(phone);
  const onVerifyOtp = async (phone, otp) => {
  ...
  if (!currentUser) {
    return (
      <>
        <LoginScreen onRequestOtp={onRequestOtp} onVerifyOtp={onVerifyOtp} t={t} />
        {tweakPanel(tweaks, setTweak)}
      </>
    );
  }

  const currentBU = businessUnits.find(u => u.id === currentUnit);
      ...
      <div className="app" data-sidebar={tweaks.sidebarCollapsed ? 'collapsed' : 'expanded'}>
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="btn ghost icon sm" onClick={onLogout} title={t('nav_logout')}><Icon name="logout" /></button>
          </div>
        </header>
        ...
        <main className="main">
          ...
          {page === 'banks' && <BanksPage store={store} setStore={setStore} businessUnits={businessUnits} currentUnit={currentUnit} t={t} toast={pushToast} confirm={confirm} />}
