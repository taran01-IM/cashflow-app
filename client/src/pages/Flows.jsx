...
import { Icon, Modal, Select, BUBadge, formatINR, formatDate, todayISO } from '../primitives.jsx';
import { CLIENTS, CATEGORIES } from '../data.js';
import { api } from '../api.js';

export function InflowPage({ store, currentUnit, setStore, t, toast, confirm }) {
  ...
  const onSave = async (data) => {
    try {
      const payload = { ...data, unit: data.unit || (currentUnit === 'all' ? 'mopl' : currentUnit) };
      const saved = data.id
        ? await api.updateInflow(data.id, payload)
        : await api.createInflow(payload);
      setStore(s => data.id
        ? { ...s, inflows: s.inflows.map(x => x.id === saved.id ? saved : x) }
        : { ...s, inflows: [saved, ...s.inflows] });
  ...
  const onDelete = (id) => confirm(t('confirm_delete'), async () => {
    try {
      await api.deleteInflow(id);
...
export function OutflowPage({ store, currentUnit, setStore, t, toast, confirm }) {
  const [editing, setEditing] = React.useState(null);
  const [search, setSearch] = React.useState('');
  ...
  const onSave = async (data) => {
    try {
      const payload = { ...data, unit: data.unit || (currentUnit === 'all' ? 'mopl' : currentUnit) };
      const saved = data.id
        ? await api.updateOutflow(data.id, payload)
        : await api.createOutflow(payload);
      setStore(s => data.id
        ? { ...s, outflows: s.outflows.map(x => x.id === saved.id ? saved : x) }
        : { ...s, outflows: [saved, ...s.outflows] });
  ...
  const toggleStatus = async (id) => {
    const row = store.outflows.find(x => x.id === id);
    if (!row) return;
    const next = row.status === 'Paid' ? 'Pending' : 'Paid';
    // Optimistic update
    setStore(s => ({ ...s, outflows: s.outflows.map(x => x.id === id ? { ...x, status: next } : x) }));
    try {
      await api.updateOutflow(id, { status: next });
    } catch (e) {
      // Roll back
      setStore(s => ({ ...s, outflows: s.outflows.map(x => x.id === id ? { ...x, status: row.status } : x) }));
...
function OutflowForm({ row, defaultUnit, onSave, onClose, t }) {
  ...
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const groupChildren = CATEGORIES.find(g => g.group === form.categoryGroup)?.children || [];
      ...
      <div className="grid-2">
        <div className="field">
          ...
          <Select value={form.categoryGroup} onChange={v => {
            const c = CATEGORIES.find(g => g.group === v);
            setForm(f => ({ ...f, categoryGroup: v, category: c?.children[0] || '' }));
          }} options={CATEGORIES.map(g => g.group)} />
        </div>
        <div className="field">
