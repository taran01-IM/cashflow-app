// Banks module — grid of bank cards + add/edit/delete

function BanksPage({ store, currentUnit, setStore, t, toast, confirm }) {
  const [editing, setEditing] = React.useState(null); // bank or 'new'
  ...
  const onSave = (data) => {
    setStore(s => {
      if (data.id) {
        return { ...s, banks: s.banks.map(b => b.id === data.id ? { ...b, ...data } : b) };
      }
      const id = 'b' + (Math.max(...s.banks.map(b => parseInt(b.id.slice(1)))) + 1);
      return { ...s, banks: [...s.banks, { ...data, id }] };
    });
    ...
    <div className="page">
      ...
      {banks.length === 0 ? (
        ...
        <div className="bank-grid">
          {banks.map(b => {
            ...
            const bu = window.CFA_DATA.BUSINESS_UNITS.find(u => u.id === b.unit);
            return (
      ...
      {editing && (
        <BankForm
          ...
          defaultUnit={currentUnit === 'all' ? 'mopl' : currentUnit}
          onSave={onSave}
...
function BankForm({ bank, defaultUnit, onSave, onClose, t }) {
    ...
    <Modal title={bank ? `${t('edit')} bank` : t('add_bank')} onClose={onClose}
      ...
      <div className="field">
        ...
        <Select value={form.unit} onChange={v => set('unit', v)}
          options={window.CFA_DATA.BUSINESS_UNITS.map(u => ({ value: u.id, label: `${u.code} — ${u.name}` }))} />
      </div>
      <div className="grid-2">
        <div className="field">
